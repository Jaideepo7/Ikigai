...
const seasonIco = (v: string) => v === 'auto' ? '🗓️' : `<img class="season-img" src="/assets/ui/season_${v}.png" alt="" />`;
...
export async function shopPanel(tab: ShopTab = 'shop', source?: HTMLElement) {
  ...
  if (tab !== 'plants' && tab !== 'cards') {
    ...
    const products = tab === 'shop' ? info.plants.map(plantCard).join('') : tab === 'seasons' ? SEASONS.filter(s => s.value !== 'auto').map(s => `<article class="market-product market-season"><span class="season-art ${s.value}">${seasonIco(s.value)}</span><b>${s.label}</b><p>${s.note}</p>${m.seasons.includes(s.value) ? '<span class="owned-label">Owned</span>' : `<button class="btn sm sage" data-season-buy="${s.value}">${ico('gem', 'sm')} ${num(u.is_admin ? 'FREE' : SEASON_CHANGE_GEMS)}</button>`}</article>`).join('') : (tab === 'outdoor' ? info.outdoor ?? [] : info.furniture).map(furnCard).join('');
    body = `<section class="market-stock"><div class="market-section-head"><div><h3>${title}</h3><p>${note}</p></div>${tab === 'shop' ? '<button class="link" data-t="plants">Flower collection &rarr;</button>' : ''}</div><div class="market-products">${products}</div></section>
      ...
      <aside class="spin-box"><h3>🎰 Daily Spin</h3><p>Spin once a day for coins and rare gems. Every spin pays at least <b>${num(spinBaseCoins(u.streak))} coins</b>.</p>
        <div class="machine v3"><img class="mach" src="/assets/ui/spin_machine.png" alt="" /><div class="reels"><div class="reel">${ico('coin', 'lg')}</div><div class="reel">${ico('sprout', 'lg')}</div><div class="reel">${ico('gem', 'lg')}</div></div>
  ...
  const spin = async () => {
    ...
    const icon: Record<Reel, string> = { coin: ico('coin', 'lg'), sprout: ico('sprout', 'lg'), gem: ico('gem', 'lg') };
    ...
    try {
      ...
      const spinner = setInterval(() => { reels.forEach((el) => { if (el.classList.contains('spinning')) el.innerHTML = [icon.coin, icon.sprout, icon.gem][Math.floor(Math.random() * 3)]; }); sfx.spin(); }, 90);
